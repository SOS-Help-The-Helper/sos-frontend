        ...
        {options.map(opt => {
          ...
          return (
            <button key={opt.id} onClick={() => onSelect(opt.id)}
              className={`relative flex flex-col items-center justify-center gap-0.5 min-h-[48px] py-2 px-1 rounded-lg border transition-all active:scale-[0.96] ${
              ...
              <span className="text-[9px] font-semibold text-white text-center leading-tight">{opt.label}</span>
            </button>
          );
        })}
      </div>
