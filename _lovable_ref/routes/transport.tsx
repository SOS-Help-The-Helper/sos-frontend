    ...
    <CrmShell module="Transport">
      ...
      <div className="px-4 md:px-6 pt-5 md:pt-6 pb-10 space-y-6">
        ...
        {view === "list" ? (
          <>
            <section>
              <p className="font-mono text-[10px] uppercase tracking-[0.14em] text-white/45 mb-3">Active transports <span className="text-white/65 tabular-nums ml-1">{transportAssignments.length}</span> <span className="text-white/30">·</span> <span className="text-white/65 tabular-nums">{convoys.length}</span> <span className="text-white/30">convoy{convoys.length === 1 ? "" : "s"}</span></p>
                  ...
                  return (
                    <div key={t.id} className="rounded-xl bg-[var(--surface-1)] border border-[var(--hairline)] overflow-hidden">
                      <button
                        ...
                        className="w-full text-left px-3.5 py-3 active:bg-white/4 transition"
                      >
                        <div className="flex items-start justify-between gap-3 mb-1.5">
                          <div className="min-w-0 flex-1">
                            <div className="flex items-center gap-1.5 mb-1">
                              <span className="w-1.5 h-1.5 rounded-full" style={{ background: STATUS_COLOR[t.status] }} />
                              <span className="text-[11.5px] text-white/75">{TRANSPORT_STATUS_LABEL[t.status]}</span>
                              <PriorityPill p={t.priority} />
                            </div>
                            <p className="font-medium text-[14px] text-white truncate">{t.driverName}</p>
                            <p className="text-[12px] text-white/65 truncate mt-0.5">{t.resourceSummary.split(" — ")[0]}</p>
                          </div>
                        ...
                        <div className="flex items-center justify-between mt-1.5 font-mono text-[10.5px] text-white/45">
                          <span>ETA {t.estimatedArrival ?? "—"}</span>
                          <span>{t.convoyId ? `${t.convoyId} · #${t.convoyPosition}` : "—"}</span>
                        </div>
                      </button>
                      {isOpen && (
                    ...
                    <div key={c.id} className="rounded-2xl bg-[var(--surface-1)] border border-[var(--hairline)] p-4">
                      <div className="flex items-center justify-between mb-2">
                        <div>
                          ...
                          <p className="font-mono text-[10px] text-white/45 mt-0.5">{c.id} · <span className="text-white/70">{org?.name}</span></p>
                        </div>
