...
export const Route = createFileRoute("/drive/$id")({
  loader: ({ params }): TransportAssignment => {
    const a = transportAssignments.find((x) => x.id === params.id);
    if (!a) throw notFound();
    return a;
  },
  head: ({ loaderData }) => ({
    meta: [
      { title: `Driver — ${loaderData?.id ?? "Trip"}` },
    ...
    <div className="min-h-screen bg-[var(--background)] text-white">
      ...
      <div className="mx-auto max-w-[480px] px-4 pt-4 pb-32">
        ...
        <header className="flex items-center justify-between mb-4">
          ...
          <span className="font-mono text-[9px] uppercase tracking-wider px-1.5 py-0.5 rounded bg-white/8 text-white/65">
            {assignment.priority}
          </span>
        </header>
        ...
        {/* Convoy badge */}
        {convoy && (
          <div className="mb-4 rounded-xl bg-white/5 border border-white/8 px-3 py-2 flex items-center gap-2.5">
        ...
        <section className="mb-4">
          <div className="flex items-center justify-between mb-2">
            ...
            <button
              onClick={onPhoto}
              className="inline-flex items-center gap-1.5 h-7 px-2.5 rounded-md bg-white/8 text-[11px] text-white/85"
            >
          ...
          {assignment.photos.length === 0 ? (
            <div className="rounded-xl border border-dashed border-white/12 px-3 py-6 text-center text-[12px] text-white/45">
              No photos yet
            </div>
          ) : (
            <div className="grid grid-cols-3 gap-2">
              {assignment.photos.map((p, i) => (
                <div
                  key={i}
                  className="aspect-square rounded-lg bg-gradient-to-br from-white/10 to-white/5 border border-white/8 flex flex-col items-center justify-center p-2"
                >
          ...
          {assignment.issues.length > 0 && (
            <div className="mb-2 space-y-1.5">
              {assignment.issues.map((it, i) => (
                <div key={i} className="rounded-lg bg-[#EF4E4B]/8 border border-[#EF4E4B]/25 px-3 py-2">
        ...
        {/* Location sharing */}
        <section className="mb-4 rounded-xl bg-white/4 border border-white/8 px-3 py-3 flex items-center gap-3">
      ...
      <div className="fixed bottom-0 left-0 right-0 z-30 glass border-t border-white/10 pb-[max(0.5rem,env(safe-area-inset-bottom))]">
        <div className="mx-auto max-w-[480px] px-4 py-2 flex items-center gap-2">
          ...
          <a
            ...
            className="flex-1 inline-flex items-center justify-center gap-1.5 h-10 rounded-lg bg-white/8 text-white/90 text-[12.5px] font-medium"
          >
            <Phone size={13} /> Call
          </a>
          <a
    ...
    <div className="fixed inset-0 z-40 bg-black/70 backdrop-blur-sm flex items-end justify-center" onClick={onClose}>
      ...
      >
        <div className="flex items-center justify-between mb-3">
          ...
          <button onClick={onClose} className="w-7 h-7 rounded-md hover:bg-white/8 flex items-center justify-center">
          ...
          {types.map((t) => (
            <button
              ...
              className={`h-7 px-2.5 rounded-md text-[11px] font-medium capitalize ${
                type === t ? "bg-[#EF4E4B]/20 text-[#EF4E4B]" : "bg-white/6 text-white/70"
              }`}
            >
              {t.replace("_", " ")}
            </button>
