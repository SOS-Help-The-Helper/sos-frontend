      ...
      <div className="px-4 pt-4 pb-4 grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-5">
        <div className="space-y-4">
          ...
          <div className="flex items-baseline gap-2 pt-1">
            <p className="font-mono text-[10px] uppercase tracking-[0.14em] text-white/55">Roster</p>
            <span className="font-mono text-[10px] tabular-nums text-white/35">{filtered.length}</span>
            <span className="font-mono text-[10px] text-white/35">·</span>
            <span className="font-mono text-[10px] tabular-nums text-white/55">{active}</span>
            <span className="font-mono text-[10px] text-white/35">active</span>
            <span className="font-mono text-[10px] text-white/35">·</span>
            <span className="font-mono text-[10px] tabular-nums text-white/55">{assigned}</span>
            <span className="font-mono text-[10px] text-white/35">on assignment</span>
          </div>
            ...
            {filtered.map((v) => {
              ...
              return (
                <button
                  ...
                  className="text-left rounded-2xl bg-[var(--surface-1)] border border-[var(--hairline)] hover:border-white/20 p-4 transition"
                >
          ...
          {/* Bottom availability grid */}
          <section className="rounded-2xl bg-[var(--surface-1)] border border-[var(--hairline)] overflow-hidden">
            ...
            <table className="w-full text-[12px]">
              <thead>
                <tr className="text-left font-mono text-[9px] uppercase tracking-wider text-white/45 border-b border-[var(--hairline)]">
        ...
        <aside className="rounded-2xl bg-[var(--surface-1)] border border-[var(--hairline)] p-5 h-fit sticky top-4">
          ...
          <div className="space-y-3">
            ...
            <Field label="Required skill"><select className="w-full h-9 rounded-md bg-white/6 border border-white/10 px-2.5 text-[12.5px]"><option>Any</option>{allSkills.map(s => <option key={s}>{s}</option>)}</select></Field>
            <Field label="Hitch type"><select className="w-full h-9 rounded-md bg-white/6 border border-white/10 px-2.5 text-[12.5px]"><option>Any</option><option>Bumper pull</option><option>5th wheel</option><option>Gooseneck</option></select></Field>
...
function VolunteerDrawer({ v, onClose }: { v: VolunteerDetail; onClose: () => void }) {
  ...
  return (
    <SideDrawer onClose={onClose}>
        ...
        <div className="flex border-b border-[var(--hairline)] px-2">
        ...
        <div className="p-5 space-y-4">
          {tab === "profile" && (
            <>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-1">{v.skills.map(s => <span key={s} className="font-mono text-[10px] px-1.5 py-0.5 rounded bg-white/6 text-white/85">{s}</span>)}</div>
              </div>
              {v.towCapacity && (
                <div>
          ...
          <Link to="/directory/volunteer/$id" params={{ id: v.id }} className="inline-flex items-center gap-1.5 h-8 px-3 rounded-lg bg-[#89CFF0]/15 hover:bg-[#89CFF0]/25 text-[#89CFF0] text-[12px] font-medium transition">
            Open full record
          </Link>
        </div>
      </SideDrawer>
