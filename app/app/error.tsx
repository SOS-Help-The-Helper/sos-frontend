...
export default function AppError({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-[#0F1E2B] text-white gap-4 p-8">
      ...
      <p className="text-sm text-white/60 max-w-md text-center">{error.message || 'Unknown error'}</p>
      {error.digest && <p className="text-xs text-white/30">Digest: {error.digest}</p>}
