                  ...
                  {pipeline.map((s, i) => {
                    ...
                    return (
                      <li key={s} className="relative">
                        <span
                          aria-label={`Status: ${TRANSPORT_STATUS_LABEL[s as TransportStatus] ?? s}`}
...
function MatchesList({ cands, onRefetch }: { cands: MatchCandidate[]; onRefetch?: () => Promise<unknown> }) {
  ...
  return (
    <div className="space-y-3">
      {cands.map((c) => {
        const isApprovingThis = actionLoading === c.id + "-approve";
        const isRejectingThis = actionLoading === c.id + "-reject";
            ...
            {!c.approved && (
              <div className="flex items-center gap-2 mt-3">
                <button
                  ...
                  disabled={isBusy}
                  className="inline-flex items-center gap-1 h-7 px-2.5 rounded-md bg-[#34D399]/15 text-[#34D399] text-[11px] font-medium hover:bg-[#34D399]/25 transition disabled:opacity-50"
                >
                  <Check size={11} /> {isApprovingThis ? "Updating…" : "Approve"}
                </button>
                <button
                  onClick={() => handleReject(c.id)}
                  disabled={isBusy}
                  className="inline-flex items-center gap-1 h-7 px-2.5 rounded-md bg-white/6 text-white/65 text-[11px] font-medium hover:bg-white/12 transition disabled:opacity-50"
                >
                  <X size={11} /> {isRejectingThis ? "Updating…" : "Reject"}
                </button>
              </div>
            )}
          </div>
  ...
  return (
    <div>
      <div className="h-1 rounded-full bg-white/8 overflow-hidden">
        <div className="h-full bg-[#89CFF0]" style={{ width: `${Math.min(100, Math.round((v / max) * 100))}%` }} />
      </div>
